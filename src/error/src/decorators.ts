...
export function AsyncTransform(context?: Partial<ErrorContextBase>) {
    return function (
        ...
        descriptor.value = async function (...args: any[]) {
            ...
            const wrappedMethod = errorTransformer.withAsyncTransform(
...
export function SyncTransform(context?: Partial<ErrorContextBase>) {
    return function (
        ...
        descriptor.value = function (...args: any[]) {
            ...
            const wrappedMethod = errorTransformer.withSyncTransform(
                originalMethod.bind(this),
                {
                    methodName: propertyKey,
                    ...context,
        ...
        return class extends constructor {
            ...
            applyAutoTransform(opts?: typeof options) {
                ...
                methods.forEach((methodName) => {
                    ...
                    if (!errorTransformer) {
                        console.warn(
                            `ErrorTransformer not found for method ${methodName} in ${this.constructor.name}`,
                        );
                        return;
                    }

                    if (isAsync) {
                        this[methodName] = errorTransformer.withAsyncTransform(
                            originalMethod.bind(this),
                            { methodName, ...opts?.defaultContext },
                        );
                    } else {
                        this[methodName] = errorTransformer.withSyncTransform(
                            originalMethod.bind(this),
                            { methodName, ...opts?.defaultContext },
                        );
                    }
