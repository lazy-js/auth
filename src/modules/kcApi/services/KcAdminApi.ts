...
export class KcAdmin extends KeycloakAdminClient {
     ...
     */
    static async create(
        createKcAdminConfig: KeycloakCreateObject,
    ): Promise<KcAdmin> {
        const { password, reAuthenticateIntervalMs, url, realmName } =
            createKcAdminConfig;
    ...
    async authenticate(password: string) {
        try {
            const { refreshExpiresIn, refreshToken, expiresIn, accessToken } =
                await getToken({
